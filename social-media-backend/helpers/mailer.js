...
exports.sendResetCode = (email, name, code) => {
  ...
  const mailOptions = {
    from: EMAIL,
    to: email,
    subject: "Reset your password for Social Media App",
    html: ` <div style=" text-align: center; border: 1px solid #f1f0f0; padding: 20px; background-color: #faf9fa; " > <h1 style="color: cadetblue; font-family: sans-serif"> Hello ${name}! Welcome to Our Service! </h1> <p style=" font-family: sans-serif; font-size: 18px; color: rgb(163, 109, 214); " > Please click the link below to verify your email address </p> <a  style=" display: inline-block; padding: 10px 20px; background-color: cadetblue; color: #efe5f3; text-decoration: none; border-radius: 5px; font-family: sans-serif; margin-top: 20px; " onMouseOver="this.style.backgroundColor='#9acfe7'; this.style.color='#000000';" onMouseOut="this.style.backgroundColor='cadetblue'; this.style.color='#efe5f3';" >
    Verify Email ${code}</a ></div>`,
  };
  stmp.sendMail(mailOptions, (err, res) => {
    if (err) return err;
    return res;
